...
def create_payment_order(student_id: int, amount: Any, currency: str, items, meta, invoice_id: Optional[int] = None):
    ...
    if invoice_id:
        ...
        # Validate invoice belongs to student and is payable
        if invoice and (invoice.student_id != int(student_id) or invoice.status == "paid"):
            # If invalid or paid, we cannot use this invoice.
            # Fallback to creating a new one (ignoring the passed ID).
            invoice = None
    
    if not invoice:
        # Cancel any existing 'created' invoices for this student to avoid double counting/billing
        # This assumes the new order supersedes previous pending ones
        existing_invoices = Invoice.query.filter_by(student_id=student.id, status="created").all()
        ...
        # Update existing invoice items if provided (optional, but good for consistency)
        if items:
            invoice.items = items
            # Also update amount if it changed? 
            # Ideally invoice amount should match.
            invoice.amount_paise = amount_paise

    client = _get_razorpay_client()
    if client:
        order = client.order.create({"amount": amount_paise, "currency": currency, "receipt": invoice.invoice_no})
    else:
        order = {"id": f"order_{uuid4().hex}", "amount": amount_paise, "currency": currency, "receipt": invoice.invoice_no}
...
def verify_payment(payload: Dict[str, Any]):
    ...
    if not payment:
        print(f"DEBUG: Payment not found for invoice_id={invoice_id} order_id={order_id}")
    ...
    print(f"DEBUG: Payment verified. Updating status for payment {payment.id} and invoice {payment.invoice_id}")
    payment.razorpay_payment_id = payment_id
    payment.status = "captured"
    
    # Explicitly update invoice status
    invoice = Invoice.query.get(payment.invoice_id)
    if invoice:
        invoice.status = "paid"
        print(f"DEBUG: Invoice {invoice.id} marked as paid")
    else:
        print(f"DEBUG: Invoice {payment.invoice_id} not found during verification")

    receipt_path = generate_receipt(invoice, payment, payment.student, current_app.config)
    payment.receipt_path = receipt_path
    db.session.commit()
    data = payment.to_dict()
    data["receiptUrl"] = f"/api/payments/{payment.invoice_id}/receipt"
    data["status"] = "success"
    return data
