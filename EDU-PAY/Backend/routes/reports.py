...
def dashboard_stats():
    # 1. Total Collection
    total_collected = (
        Payment.query.filter_by(status="captured")
        .with_entities(func.coalesce(func.sum(Payment.amount_paise), 0))
        .scalar()
        or 0
    )

    # 2. Students Paid vs Not Paid & Pending Dues
    students = Student.query.all()
    pending_dues = 0
    students_paid_count = 0
    students_not_paid_count = 0

    for student in students:
        outstanding = student.outstanding_amount()
        if outstanding > 0:
            pending_dues += outstanding
            students_not_paid_count += 1
        else:
            students_paid_count += 1

    # 3. Collection by Course
    course_breakdown = (
        db.session.query(Student.course, func.coalesce(func.sum(Payment.amount_paise), 0))
        .join(Payment, Payment.student_id == Student.id)
        .filter(Payment.status == "captured")
    ...
    by_course = [
        {"course": course or "Unknown", "amount": amount}
        for course, amount in course_breakdown
    ]

    # 4. Collection by Year
    year_breakdown = (
        db.session.query(func.strftime('%Y', Payment.created_at), func.coalesce(func.sum(Payment.amount_paise), 0))
        .filter(Payment.status == "captured")
        .group_by(func.strftime('%Y', Payment.created_at))
        .all()
    )
    by_year = [
        {"year": year, "amount": amount}
        for year, amount in year_breakdown
    ]

    data = {
        "totalCollection": total_collected / 100,
        "pendingDues": pending_dues / 100,
        "studentsPaid": students_paid_count,
        "studentsNotPaid": students_not_paid_count,
        "collectionByCourse": [{"course": i["course"], "amount": i["amount"] / 100} for i in by_course],
        "collectionByYear": [{"year": i["year"], "amount": i["amount"] / 100} for i in by_year]
    }
    return json_response(True, data)
